...
// Usuarios predefinidos (se inicializan en AsyncStorage si no existen)
...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        PREDEFINED_USERS.forEach((pre) => {
          const exists = users.find((u) => u.email === pre.email);
          if (!exists) users.push(pre);
        });
  ...
  const animateTransition = useCallback(
    (direction: number, nextScreen: ScreenKey) => {
      ...
      Animated.parallel([
        ...
        slideAnim.setValue(
          direction === 2 ? SLIDE_SMALL : direction === -1 ? -SLIDE_LARGE : SLIDE_LARGE
        );
        Animated.parallel([
  ...
  const changeScreen = useCallback(
    (nextScreen: ScreenKey) => {
      ...
      else if (nextScreen === SCREENS.LOGIN) animateBackground(CONFIG.bg.resetX, 0, 400);
      ...
      <View style={styles.container}>
        <Animated.View style={[styles.card, { transform: getTransform(), opacity: opacityAnim }]}>
          {screen === SCREENS.LOGIN && (
            <LoginScreen onRegister={() => changeScreen(SCREENS.REGISTER)} onSuccess={() => router.push("/home")} />
          )}
          ...
          {screen === SCREENS.VERIFY && (
            <VerifyScreen user={tempUser} onSuccess={() => router.push("/home")} onBack={() => changeScreen(SCREENS.LOGIN)} />
          )}
        </Animated.View>
      </View>
    </View>
...
function LoginScreen({
  ...
  const handleLogin = async () => {
    ...
    try {
      const saved = await AsyncStorage.getItem("users");
      const users = saved ? (JSON.parse(saved) as User[]) : [];
